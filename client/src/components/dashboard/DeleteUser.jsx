...
const DeleteUser = (props) => {
  ...
  const fetchData = async () => {
      try{
          const api_call = `http://localhost:8000/api/getalluserdets`;
          ...
          if (!response.ok){
              setMsg("Couldnt Retrieve User Details")
        ...
        {
          setMsg("Couldnt Retrieve User Details! Try after sometime")
  ...
  const submit = async (e) => {
    ...
    try{
        ...
        //Response statuscode - not in range 200-299 : Handle error
        if (!response.ok){
            setMsg("Couldnt Delete User! User has other dependencies")
            throw new Error(`HTTP error! Status: ${response.status}`); //Code
        }  
        setMsg("User has been deleted Successfully!")
  ...
  return (
    <div className='absolute left-[20%] w-[80%] h-[90%] pt-20 px-4 justify-center items-center'>
        <h3 className='text-4xl font-bold text-center pb-2 text-dark-blue'>DELETE USERS - {props.role}</h3> 
        ...
        <div className='w-[60%] h-[55%] absolute left-[18%] flex flex-col m-6 shadow-xl shadow-slate-200 p-8 rounded-lg bg-logo-blue max-h-150 overflow-y-auto'>
            ...
            <table className='justify-center items-center text-center text-white text-bold mt-6 '>
                    <tr className='text-2xl '>
                        ...
                        <th className='p-3 border border-gray-300 w-1/3'>Email ID</th>
                    </tr>
                    {data && data.map((item) => (  
                    <tr key={item.username} className='text-xl text-white' >
                        ...
                        <td className="p-3 border border-gray-300 w-1/3">{item.email_id}</td>
                    </tr> 
                    ))}
            </table>   
        </div>  
