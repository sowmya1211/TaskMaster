...
const AssignTask = (props) => {
  ...
  const fetchData = async () => {
      try{ 
          const api_call = `http://localhost:8000/api/getnontaskgroupuserdets/${group_name}/${task_name}`;
  ...
  const submit = async (e) => {
    ...
    try{
        const api_call = `http://localhost:8000/api/assigntask/${group_name}/${task_name}/${username}`
        const response = await fetch(api_call, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            credentials: 'include', //To communicate cookies with server
        ...
        //Response statuscode - not in range 200-299 : Handle error
        if (!response.ok){
            setMsg("Couldnt Assign Task to the user!")
            throw new Error(`HTTP error! Status: ${response.status}`); //Code
        }  
        setMsg("Task has been assigned to User Successfully!")
  ...
  return (
    <div className='absolute left-[20%] w-[80%] h-[90%] pt-20 px-4 justify-center items-center'>
        <h3 className='text-4xl font-bold text-center pb-2 text-dark-blue'>ASSIGN TASK TO USERS - {task_name} </h3> 
        <h3 className='text-xl font-serif font-bold text-center pb-1 text-dark-blue '>{msg}</h3> 
        <h3 className='text-xl font-bold text-center pb-1 text-dark-blue'>NOTE: Enter a user and click assign</h3> 
